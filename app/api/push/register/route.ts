...
import { PUSH_TOKENS_SUBCOLLECTION } from "@/lib/push/constants"
...
export async function POST(request: NextRequest) {
  ...
  try {
    // El id del documento es el token, así que reabrir la app sólo refresca
    // updatedAt en lugar de acumular registros del mismo dispositivo.
    await tokenDoc(uid, token).set(
...
export async function DELETE(request: NextRequest) {
  ...
  try {
    await tokenDoc(uid, token).delete()
    return NextResponse.json({ ok: true })
  } catch (error) {
