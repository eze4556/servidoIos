...
export function StoriesRail({ groups, loading, onOpenAuthor, className }: StoriesRailProps) {
  ...
  // Índices reales en `groups` para abrir el visor
          ...
          {hasOwnStories ? (
            <>
              <div className="relative">
                <button
                  type="button"
                  onClick={() => onOpenAuthor(myGroupIndex)}
                    ...
                    <span className="relative h-full w-full overflow-hidden rounded-full bg-purple-100">
                      {photo ? (
                        ...
                        <span className="flex h-full w-full items-center justify-center text-sm font-bold text-servido-800">
                          {(currentUser.firebaseUser.displayName || "Y").charAt(0).toUpperCase()}
                        </span>
                      )}
                    </span>
                  </span>
            ...
            <Link href="/historias/nueva" className="flex w-full flex-col items-center gap-1.5">
              <span className="relative flex h-14 w-14 items-center justify-center overflow-hidden rounded-full bg-gradient-to-br from-servido-700 to-servido-950 text-white shadow-md ring-2 ring-white">
                {photo && (
                  // eslint-disable-next-line @next/next/no-img-element
